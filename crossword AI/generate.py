...
class CrosswordCreator:
    ...
    def revise(self, x, y):
        ...
        x_overlap_index, y_overlap_index = overlaps
        words_to_remove = set()
        for x_word in self.domains[x]:
            if not any(x_word[x_overlap_index] == y_word[y_overlap_index] for y_word in self.domains[y]):
    ...
    def backtrack(self, assignment):
        ...
        # Order the domain values based on least-constraining values
        for value in self.order_domain_values(var, assignment):
            new_assignment = assignment.copy()
            new_assignment[var] = value
            if self.consistent(new_assignment):
                self.ac3([(n, var) for n in self.crossword.neighbors(var)])
